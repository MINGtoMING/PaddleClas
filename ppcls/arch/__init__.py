...
from .gears import build_gear, MLDecoder
from .utils import *
...
def build_model(config, mode="train"):
    ...
    if use_ml_decoder:
        ml_decoder_config = copy.deepcopy(config["MLDecoder"])
        ...
        ml_decoder = MLDecoder(**ml_decoder_config)
        for layer_name, replace_layer in zip(["avg_pool", "flatten", "fc"],
                                             [nn.Identity(), nn.Identity(), ml_decoder]):

            if hasattr(arch, layer_name):
                delattr(arch, layer_name)
                setattr(arch, layer_name, replace_layer)
            else:
