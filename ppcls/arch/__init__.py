...
def build_model(config, mode="train"):
    ...
    if use_ml_decoder:
        head_config = config.get("MLDecoder", {})
        if "class_num" not in head_config:
            if hasattr(arch, "class_num"):
                head_config["class_num"] = arch.class_num
            else:
                head_config["class_num"] = arch_config["class_num"]
        add_ml_decoder_head(arch, head_config)
